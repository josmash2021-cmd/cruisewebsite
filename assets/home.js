...
(function () {
  ...
  /* ---- coreografía del hero: contenido cambia en el 34,7s
         y el vídeo se pausa en el 39s (dos eventos distintos) ---- */
  var hero = document.querySelector('.hero');
  var heroVideo = document.querySelector('.hero-video');
  if (hero && heroVideo) {
    var phase2Done = false;
    var pausedDone = false;
    var checkPhase = function () {
      if (!phase2Done && heroVideo.currentTime >= 34.7) {
        phase2Done = true;
        hero.classList.add('hero--phase2');
      }
      if (!pausedDone && heroVideo.currentTime >= 39) {
        pausedDone = true;
        heroVideo.pause();                 /* el vídeo se congela en el segundo 39 */
      }
    };
    heroVideo.addEventListener('timeupdate', checkPhase);
    /* si el vídeo no llega a cargar/reproducir, no hacemos nada:
       la fase 1 permanece visible */
  }
